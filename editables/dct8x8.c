/**
* file dct8x8.cu: main CUDA program
* it Contains entry point, wrappers to host and device code and benchmark.
*
* This sample implements forward and inverse Discrete Cosine Transform to blocks
* of image pixels (of 8x8 size), as in JPEG standard. 
...
* Gold version refers to the code executed sequentially on the CPU
...
//This file Contains declaration of CPU versions of DCT, IDCT and quantization routines
#include "DCT8x8_Gold.h"

//This file contains declaration of basic bitmap loading, saving, conversions to different representations and memory management routines.
#include "BmpUtil.h"

/**
*  we are declaring the number of DCT kernel calls
*/
...
/**
**************************************************************************
*  Wrapper function for  gold version of DCT, quantization and IDCT implementations
*
...
*
* the function returns Execution time in milliseconds
*/
...
/**
*  Wrapper function for CUDA version of DCT, quantization and IDCT implementations
*
* \param ImgSrc         [IN] - Source byte image plane
* \param ImgDst         [IN] - Quantized result byte image plane
* \param Stride         [IN] - Stride for both source and result planes
...
int main(int argc, char **argv)
{
    ...
    //initialize CUDA and find the respective device 
	...
	//in case the requested image is not found
	...
	//in case that the file is not an image
    if (res)
    {
    ...
    //check image dimensions are multiples of BLOCK_SIZE, we need image sizes that are multiple of 8, in case of incorrect size we execute the following 
    if (ImgWidth % BLOCK_SIZE != 0 || ImgHeight % BLOCK_SIZE != 0)
    {
        printf("\nError: Input image dimensions must be multiples of 8!\n");
        exit(EXIT_FAILURE); //we abort the program if we enter this statement 
        return 1;
    }
    ...
    //compute Gold 1 (CPU) version of DCT/quantization/IDCT
    printf("Success\nRunning Gold 1 (CPU) version... ");
    float TimeGold1 = WrapperGold1(ImgSrc, ImgDstGold1, ImgStride, ImgSize);

    //compute CUDA 1 (GPU) version of DCT/quantization/IDCT
	...
	printf("Processing time (GOLD 1) CPU (Sequential)  : %f ms \n", TimeGold1);
    printf("Processing time (CUDA 1) GPU (Parallel)    : %f ms \n", TimeCUDA1);
